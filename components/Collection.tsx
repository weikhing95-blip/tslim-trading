...
import GradeCarousel from './GradeCarousel'

function getGradeImages(slug: string): string[] {
  const dir = path.join(process.cwd(), 'public/assets/photos/grades', slug)
  try {
    return fs.readdirSync(dir)
      .filter(f => /\.(jpe?g|png|webp)$/i.test(f))
      .sort()
      .map(f => `/assets/photos/grades/${slug}/${f}`)
  } catch {
    return []
  }
}
...
const grades = [
  {
    ...
    imgAlt: 'Manek Kasar — coarse bead',
  },
  {
    slug: 'manek-halus',
    tag: 'Grade II · Fine',
    ...
    <section className="section" id="collection">
      <div className="container">
        ...
        <div className="grades">
          {grades.map((g) => {
            const images = getGradeImages(g.slug)
            return (
              <article className="grade-card" key={g.title}>
                <div className="grade-img">
                  <GradeCarousel images={images} alt={g.imgAlt} />
                </div>
